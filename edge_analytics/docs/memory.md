...
| 8G | visualization | 3 artifacts: waveforms + block diagram + **Vivado/Yosys synthesized schematic**. **✅ ALL DONE.** (a) Synth via Yosys on Mac (`synthesis/`: top-block + moving_avg schematics + FPGA util = ~1245 LUT / 1163 FF / **3 DSP** = ~6% of Artix-7 xc7a35t; `SYNTHESIS_REPORT.md`). (b) **Waveforms ✅** rendered from real `dump.vcd` by a pure-Python VCD→SVG renderer (no gtkwave needed): `waveform_anomaly_radio.{svg,png}` (TEDA→radio) + `waveform_pump_control.{svg,png}` (smoothing→pump). (c) **Pump-hysteresis FSM ✅** `fsm_pump.{svg,png}`. Clean block diagram `architecture.{svg,png}`. ⚠️ NO Fmax/power (Yosys ≠ P&R — don't fabricate). | ✅ done (synth + waveforms + FSM + block diagram) |
