...
- **③ Two-tier triage** — Tier-1 acts locally (pump/doser); Tier-2 stays **silent** and sends the caretaker **one tiny alert only when a human is truly needed** — **98% fewer transmissions** for the same outcome.
- **④ It's real silicon** — synthesizable RTL, proven in simulation, maps to a real FPGA.

**Image:** a screenshot of `demo/mission_control.html` (the Caretaker's Phone + the "dumb vs our chip" counter mid-race).
...
- **Metrics (real):** **98% fewer caretaker transmissions** (6 packets vs 223 samples); **~1,245 LUTs / 1,163 FFs / 3 DSP blocks → ~6% of an Artix-7 FPGA**.
...
- **Mission Control** (`demo/mission_control.html`) — a live animated dashboard replaying the **real** 223-sample sim: raw-vs-smoothed charts, pump/alerts, TEDA anomaly, fusion, the **Caretaker's Phone**, and the **dumb-node-vs-our-chip** counter ending at **223 vs 6 → 98%**.
...
5. **Sensor anomaly** → TEDA flags it → **Caretaker's Phone buzzes**: "CHECK SENSOR."
6. End on the counter: **"223 transmissions for a dumb node, 6 for ours — 98% fewer, same crop outcome. That silence is why it runs at the edge."**

### Grill Q&A (rehearse these)
- **"It's too common / just automation."** → "Automation is only Tier-1. The novelty is on-chip *triage* — the chip decides *whether a human is needed* and self-tunes per field. That's 98% less radio traffic and it's in dedicated silicon, which no surveyed system does."
...
- **"Your dashboard streams every sample — where's the saving?"** → "Two different links. The dashboard is the cheap on-site/wired link. The 98% applies to the long-range battery caretaker radio — that's the one that stays silent."
